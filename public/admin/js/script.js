...
if (uploadListImage) {
  const uploadListImageInput = document.querySelector(
    "[upload-list-image-input]"
  );
  const uploadListImagePreview = document.querySelector(
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (sidebarToggle) {
    ...
    console.warn("Element with ID 'sidebarToggle' not found in the DOM");
  }

  // Phần còn lại của code
  try {
    // Đánh dấu menu item đang active
    ...
    const pageTitleText = pageTitle
      ? pageTitle.innerText.split(" | ")[0]
      : "Dashboard";
    const pageTitleElement = document.querySelector(".page-title");
    if (pageTitleElement) {
      pageTitleElement.innerText = pageTitleText;
    }
  } catch (err) {
    console.error("Error in admin JavaScript:", err);
  }
});
